...
$css_content = file_get_contents(__DIR__ . '/generated-article-ip-advance-return-guarantee.html');

// Извлекаем CSS из HTML файла
preg_match('/<style>(.*?)<\/style>/s', $css_content, $matches);
$css_styles = isset($matches[1]) ? $matches[1] : '';
...
if ($existing_post) {
    ...
    $updated_content = '<style>' . $css_styles . '</style>' . $current_content_cleaned;
