...
echo "=== ЗАМЕНА ФАВИКОНОВ НА ПРАВИЛЬНЫЕ ИЗОБРАЖЕНИЯ ===\n\n";
...
foreach ($matches as $match) {
    ...
    // Проверяем, есть ли уже правильное изображение
    if ($current_thumbnail_id == $correct_image_id) {
        $image_path = get_attached_file($current_thumbnail_id);
        if ($image_path && file_exists($image_path) && strpos($image_path, 'favicon') === false) {
            echo "✓ Статья ID $post_id ($post_date): '$post_title' - уже имеет правильное изображение ID $correct_image_id.\n";
    ...
    // Проверяем и исправляем пути к файлам изображения, если они некорректны
    $image_path = get_attached_file($correct_image_id);
    $upload_dir = wp_upload_dir();
    $base_dir = $upload_dir['basedir'];
    $base_url = $upload_dir['baseurl'];
    
    if (!$image_path || !file_exists($image_path)) {
        // Попытка найти WebP версию файла, если текущий путь неверен
        $image_post = get_post($correct_image_id);
        $original_guid = $image_post->guid;
        $original_file_meta = get_post_meta($correct_image_id, '_wp_attached_file', true);
        
        $filename_parts = pathinfo($original_file_meta);
        $filename_without_ext = $filename_parts['filename'];
        $dirname = $filename_parts['dirname'];
        
        $possible_webp_path_full = $base_dir . '/' . $dirname . '/' . $filename_without_ext . '.webp';
        $possible_webp_path_meta = $dirname . '/' . $filename_without_ext . '.webp';
        
        if (file_exists($possible_webp_path_full)) {
            update_post_meta($correct_image_id, '_wp_attached_file', $possible_webp_path_meta);
            wp_update_post(array(
                'ID' => $correct_image_id,
                'guid' => str_replace($base_dir, $base_url, $possible_webp_path_full)
            ));
            echo "   -> Исправлен путь к файлу для изображения ID $correct_image_id на WebP.\n";
        } else {
    ...
    echo "✔ Заменен фавикон на правильное изображение для статьи ID $post_id ($post_date): '$post_title' (Изображение ID: $correct_image_id)\n";
    $fixed_count++;
}
