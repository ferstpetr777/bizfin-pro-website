...
echo "=== ИСПРАВЛЕНИЕ СТАТЬИ АФФИНИТИ-КАРТЫ ===\n\n";

// 1. Исправляем главное изображение
$correct_image_id = 2688; // Правильное изображение
set_post_thumbnail($post_id, $correct_image_id);

// Получаем URL правильного изображения
$image_url = wp_get_attachment_url($correct_image_id);
$image_alt = get_post_meta($correct_image_id, '_wp_attachment_image_alt', true);

// 2. Извлекаем блок содержания
$table_of_contents = '';
if (preg_match('/(<div[^>]*>.*?<\/div>)/s', $content, $matches)) {
    $table_of_contents = $matches[1] . "\n\n";
}

// 3. Создаем правильный блок изображения
$image_block = '<!-- wp:image {"id":' . $correct_image_id . ',"sizeSlug":"large","linkDestination":"none","className":"ios-style-image alignwide"} -->
<figure class="wp-block-image size-large ios-style-image alignwide"><img decoding="async" width="1024" height="1024" src="' . esc_url($image_url) . '" alt="' . esc_attr($image_alt) . '" class="wp-image-' . $correct_image_id . '"/></figure>
<!-- /wp:image -->';

// 4. Извлекаем основной текст (после блока содержания и изображения)
...
if ($text_start !== false) {
    ...
    echo "✓ Установлено правильное изображение ID $correct_image_id\n";
