...
// Получаем все статьи, созданные 7 октября 2025
...
echo "=== ПРОВЕРКА РАБОТОСПОСОБНОСТИ ИЗОБРАЖЕНИЙ ДЛЯ СТАТЕЙ ОТ 7 ОКТЯБРЯ ===\n\n";
...
foreach ($posts as $post_id) {
    ...
    if ($image_path && file_exists($image_path)) {
        $working_images++;
        echo "✓ Статья ID $post_id: '$post_title' - изображение работает (путь: $image_path)\n";
    } else {
        $broken_images++;
