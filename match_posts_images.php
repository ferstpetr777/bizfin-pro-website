...
foreach ($posts as $post_id) {
    ...
    foreach ($images as $image_id) {
        ...
        if ($post_title === $image_title) {
            ...
            echo "✓ СОВПАДЕНИЕ: Статья ID $post_id -> Изображение ID $image_id\n";
    ...
    if (!$found_match) {
        echo "✗ НЕТ СОВПАДЕНИЯ: Статья ID $post_id\n";
        echo "  Название: '$post_title'\n\n";
        $unmatched++;
    }
}
...
foreach ($matches as $match) {
    echo "Статья ID {$match['post_id']} -> Изображение ID {$match['image_id']}: '{$match['title']}'\n";
}
