...
echo "=== ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ ФАВИКОНОВ ===\n\n";

foreach ($matches as $match) {
    $post_id = $match['post_id'];
    $current_image_id = $match['image_id'];
    
    $total_processed++;
    $post_title = get_the_title($post_id);
    $post_date = get_the_date('Y-m-d', $post_id);
    
    // Проверяем, указывает ли текущее изображение на фавикон
    ...
    if (!empty($correct_images)) {
        ...
        // Проверяем, что найденное изображение не является фавиконом
        if ($correct_file && file_exists($correct_file) && stripos($correct_file, 'cropped-Бизнес-Финанс-—-фавикон') === false) {
            ...
            update_post_meta($current_image_id, '_wp_attached_file', $correct_file_meta);
            wp_update_post(array(
                'ID' => $current_image_id,
                'guid' => $correct_guid
            ));
            wp_update_attachment_metadata($current_image_id, $correct_metadata);
            
            echo "✔ Исправлен фавикон для статьи ID $post_id ($post_date): '$post_title' (скопированы метаданные с ID $correct_image_id)\n";
            $fixed_count++;
        } else {
            echo "✗ Найденное изображение для статьи '$post_title' также является фавиконом\n";
            $not_found_count++;
        }
    } else {
        echo "✗ Не найдено подходящее изображение для статьи '$post_title'\n";
        $not_found_count++;
    }
}

echo "\n=== РЕЗУЛЬТАТЫ ФИНАЛЬНОГО ИСПРАВЛЕНИЯ ===\n";
